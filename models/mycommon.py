...
class Conv_new(nn.Module):
    ...
    def forward(self, x):
        df = pd.read_excel('')
        arr = np.array(df)  # offload tensor id list
        ...
        if i in arr:    # offload tensor i-1 
            ...
            x = self.conv(x)
        print("finished layers:",count)

        i += 1  # tensor id
        if i in arr:    # offload conv
            with torch.autograd.graph.save_on_cpu():
                x = self.bn(x)
        else:
            x = self.bn(x)
...
class Conv_in110(nn.Module):
    # offload input,conv,bn
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
        self.bn = nn.BatchNorm2d(c2)
...
class Conv_in(nn.Module):
    ...
    def forward(self, x):
        with torch.autograd.graph.save_on_cpu():
            res = self.conv(x)
        res1 = self.act(self.bn(res))
        return res1

    def forward_fuse(self, x):
        return self.act(self.conv(x))

class Conv_100(nn.Module):
    # offload conv
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, act=True):  # ch_in, ch_out, kernel, stride, padding, groups
        super().__init__()
        self.conv = nn.Conv2d(c1, c2, k, s, autopad(k, p), groups=g, bias=False)
...
class Bottleneck_000100(nn.Module):
    # offload the conv of cv2
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv_100(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))

class Bottleneck_001110(nn.Module):
    # offload the bn,act of cv1 & the conv,bn of cv2
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
        self.cv2 = Conv_in110(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2

    def forward(self, x):
        return x + self.cv2(self.cv1(x)) if self.add else self.cv2(self.cv1(x))

class Bottleneck_in111110(nn.Module):
    # offload the  input,cv1,and the conv,bn of cv2
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv_in110(c1, c_, 1, 1)
        self.cv2 = Conv_in110(c_, c2, 3, 1, g=g)
        self.add = shortcut and c1 == c2
...
class Bottleneck_001110(nn.Module):
    # offload the  act 0f cv1 and conv,bn of cv2
    def __init__(self, c1, c2, shortcut=True, g=1, e=0.5):  # ch_in, ch_out, shortcut, groups, expansion
        super().__init__()
        c_ = int(c2 * e)  # hidden channels
        self.cv1 = Conv(c1, c_, 1, 1)
...
class SPP_8(nn.Module):
    # Module[8] offload cv1/cv2
    def __init__(self, c1, c2, k=(5, 9, 13)):
        super().__init__()
        c_ = c1 // 2  # hidden channels
        self.cv1 = Conv_1(c1, c_, 1, 1)
